...
class VirusTotal(commands.Cog):
    ...
    async def submit_attachment_for_analysis(self, ctx, session, vt_key, attachment):
        ...
        async with session.get(attachment.url) as response:
            ...
            await self.send_info(
                ctx,
                "Starting analysis",
                "This could take a few minutes, please be patient. You'll be mentioned when results are available.",
            )
            # Fix: Use aiohttp's FormData for file upload
            form = aiohttp.FormData()
            form.add_field("file", file_content, filename=file_name)
        ...
        async with aiohttp.ClientSession() as session:
            try:
                ...
                if sha256 and sha1 and md5:
                    await self.send_analysis_results(
                        ctx, presid, sha256, sha1, file_name, malicious_count, total_count, percent, safe_count
                    )
                    self.log_submission(
                        ctx.author.id,
                        f"`{file_name or file_url}` - **{malicious_count}/{total_count}** - [View results](https://www.virustotal.com/gui/file/{sha256})",
                    )
                else:
    ...
    async def send_analysis_results(
        self, ctx, presid, sha256, sha1, file_name, malicious_count, total_count, percent, safe_count
    ):
        content = f"||<@{presid}>||"
        guild = getattr(ctx, "guild", None)
        channel = getattr(ctx, "channel", None)
        ...
        if can_embed:
            ...
            if malicious_count >= 11:
                ...
                embed.description = (
                    f"**{int(percent)}%** of vendors rated this file dangerous! You should avoid this file completely, and delete it from your systems to ensure security."
                )
                ...
                embed.title = "Analysis complete"
                embed.description = (
                    f"**{int(percent)}%** of vendors rated this file dangerous. While there are malicious ratings available for this file, there aren't many, so this could be a false positive. **You should investigate further before coming to a decision.**"
                )
    ...
    async def submission_history(self, ctx):
        ...
        if user_id in self.submission_history and self.submission_history[user_id]:
            ...
            embed = discord.Embed(
                title="Your recent VirusTotal submissions",
                description=history,
                colour=discord.Colour(0x2BBD8E),
            )
        else:
            embed = discord.Embed(
                title="No recent submissions",
                description="You have not submitted any files for analysis yet. Submissions reset when the bot restarts.",
                colour=discord.Colour(0xff4545),
            )
        await ctx.send(embed=embed)
